...
const PROGRAMS = {
    ...
    uniform vec2 u_pos;
    uniform float u_r;
    uniform float u_brush; // 0.0: clear, 1.0: original seed, 2.0: color draw
    uniform vec4 u_draw_color; // RGBA color for drawing
...
export function createCA(gl, layerWeights, gridSize) {
    ...
    function paint(x, y, r, brush, color_hex) {
        ...
        uniforms.u_brush = brush_map[brush] || 0.0;
        
        if (brush === 'color' && color_hex) {
            const r = parseInt(color_hex.slice(1, 3), 16) / 255.0;
            const g = parseInt(color_hex.slice(3, 5), 16) / 255.0;
